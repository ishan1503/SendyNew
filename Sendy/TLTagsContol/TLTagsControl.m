...
- (void)deleteTagButton:(UIButton *)sender {
    ...
    self.languages=[[self tags] componentsJoinedByString:@","];
...
-(BOOL)textFieldShouldBeginEditing:(UITextField *)textField
{
    if(self.tag!=1009089)
    {
        UIView *view=(UIView *)[[[[[[textField superview] superview] superview] superview] superview] superview];
        
        UITableViewCell *cell=(UITableViewCell *)[[textField superview] superview];
        UITableView *tableView=(UITableView *)[[[[[textField superview] superview] superview] superview] superview];
        NSIndexPath *indexPath=[tableView indexPathForCell:cell];
    //    if(IS_IPHONE_4_OR_LESS)
    //    {
            [view setFrame:CGRectMake(view.frame.origin.x, view.frame.origin.y-60*(indexPath.row+1),view.frame.size.width,view.frame.size.height)];
       // }
        return YES;
...
- (void)alertView:(UIAlertView *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    {
        NSString *temp=[[self tags] componentsJoinedByString:@","];
        NSString *newString;
        if(self.tags.count>0)
            newString=[[temp stringByAppendingString:@","] stringByAppendingString:[actionSheet textFieldAtIndex:0].text];
        else
            newString=[actionSheet textFieldAtIndex:0].text;
        self.tags=[NSMutableArray arrayWithArray:[newString componentsSeparatedByString:@","]];
        [self reloadTagSubviews];
    }
    else
        [[actionSheet textFieldAtIndex:0] resignFirstResponder];
}
...
-(void)resetTableViewDown
{
    if(self.tag!=1009089)
    {
        UIView *view=(UIView *)[[[[[[tagInputField_ superview] superview] superview] superview] superview] superview];
